...
public class ChatDAO extends BaseDAO<Chat> {
    ...
    public List<User> getChatPartners(Integer userId) {
        String hql = "SELECT DISTINCT c.pengirim FROM Chat c " +
                     "WHERE c.penerima.idUser = :userId " +
                     "UNION " +
                     "SELECT DISTINCT c.penerima FROM Chat c " +
                     "WHERE c.pengirim.idUser = :userId " +
                     "ORDER BY c.waktuPesan DESC";
        return listByQuery(hql, Map.of("userId", userId), User.class);
    }
